...
double getValidatedDouble(const string& prompt, double minValue = 0.0, double maxValue = 1000000.0) {
    double value;
    while (true) {
        cout << prompt;
        if (!(cin >> value)) {
        ...
        if (value < minValue) {
            cout << Color::RED << "Value must be at least " << minValue << "! Please enter again: " << Color::RESET;
        }
        else if (value > maxValue) {
            cout << Color::RED << "Value must be at most " << maxValue << "! Please enter again: " << Color::RESET;
        }
        else {
            return value;
        }
...
int getValidatedInt(const string& prompt, int minValue = 0, int maxValue = 1000000) {
    int value;
    while (true) {
        cout << prompt;
        if (!(cin >> value)) {
...
int getValidatedAge(const string& prompt) {
    return getValidatedInt(prompt, 1, 120);
}
...
public:
    ...
    void setUsername(string username) {
        ...
        if (!hasLetter)
            throw string(Color::RED + "Username must contain at least one letter!" + Color::RESET);
...
public:
    ...
    void saveToFile() {
        try {
            ...
            if (!f.is_open()) {
                throw runtime_error("Could not open ingredients.txt file!");
            }
            for (size_t i = 0; i < ingredients.size(); i++) {
            ...
            cout << Color::RED << "Error saving ingredients: " << e.what() << Color::RESET << endl;
        }
    }
    void loadFromFile() {
        try {
            ...
            cout << Color::RED << "Error loading ingredients: " << e.what() << Color::RESET << endl;
        }
    }
};
...
public:
    ...
    void setName(string n) {
        ...
        // Əlavə validation - ən azı 1 hərf olmalıdır
        bool hasLetter = false;
        ...
        if (!hasLetter) {
            throw string(Color::RED + "Dish name must contain at least one letter!" + Color::RESET);
        }
...
public:
    ...
    void saveToFile(const OrderRecord& rec) {
        try {
            ...
            if (!f.is_open()) {
                throw runtime_error("Could not open orders.txt file!");
            }
            ...
            cout << Color::RED << "Error saving order: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void showUserHistory(const string& username) {
        try {
            ...
            if (!f.is_open()) {
                cout << Color::YELLOW << "No order history found!" << Color::RESET << endl;
                return;
            }
            string line;
            ...
            cout << Color::RED << "Error loading order history: " << e.what() << Color::RESET << endl;
        }
    }
};
...
public:
    ...
    void saveBudget() {
        try {
            ...
            cout << Color::RED << "Error saving budget: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void addIngredientToStock() {
        ...
        // Böyük-kiçik hərf fərqi olmadan yoxla
        Ingredient* existingIng = stock.findByName(n);
        if (existingIng != NULL) {
            throw string(Color::RED + "Ingredient '" + n + "' already exists in stock! (Existing: " + existingIng->getName() + ")" + Color::RESET);
        }
        ...
        price = getValidatedDouble(Color::CYAN + "Price per kg (AZN): " + Color::RESET, 0.01, 10000);
        qty = getValidatedDouble(Color::CYAN + "Quantity (kg): " + Color::RESET, 0.01, 10000);
    ...
    void increaseExistingIngredient() {
        ...
        qty = getValidatedDouble(Color::CYAN + "Quantity to add (kg): " + Color::RESET, 0.01, 10000);
    ...
    Dish createDish() {
        ...
        while (!validName) {
            ...
            try {
                ...
                // Eyni adlı yeməyin olub-olmadığını YOXLA (case-insensitive)
                for (size_t i = 0; i < menu.size(); i++) {
                    if (equalsIgnoreCaseASCII(menu[i].getName(), n)) {
                        throw string(Color::RED + "Dish with name '" + n + "' already exists in menu! (Existing: " + menu[i].getName() + ")" + Color::RESET);
                    }
                }
        ...
        p = getValidatedDouble(Color::CYAN + "Price (AZN): " + Color::RESET, 0.01, 10000);
        ...
        count = getValidatedInt(Color::CYAN + "How many ingredients?: " + Color::RESET, 2, 20);
        ...
        for (int i = 0; i < count; i++) {
            ...
            used = getValidatedDouble(Color::CYAN + "Quantity (kg): " + Color::RESET, 0.001, 100);
    ...
    void addDishToMenu() {
        ...
        // Əlavə təhlükəsizlik üçün yenidən YOXLA (case-insensitive)
        for (size_t i = 0; i < menu.size(); i++) {
            if (equalsIgnoreCaseASCII(menu[i].getName(), d.getName())) {
                throw string(Color::RED + "Dish with name '" + d.getName() + "' already exists! (Existing: " + menu[i].getName() + ")" + Color::RESET);
            }
        }

        // İngredientlərin stock-da olub-olmadığını yoxla
        vector<IngredientUsage> ingredients = d.getIngredients();
        for (size_t i = 0; i < ingredients.size(); i++) {
            Ingredient* ing = stock.findByName(ingredients[i].ingredient->getName());
            if (ing == NULL) {
                throw string(Color::RED + "Ingredient '" + ingredients[i].ingredient->getName() + "' is not in stock!" + Color::RESET);
            }
        }
    ...
    void saveMenuToFile() {
        try {
            ofstream f("menu.txt");
            if (!f.is_open()) {
                throw runtime_error("Could not open menu.txt file!");
            }
            for (size_t i = 0; i < menu.size(); i++) {
            ...
            cout << Color::RED << "Error saving menu: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void loadMenuFromFile() {
        try {
            ...
            menu.clear();
            string line, name, info; double price;
            vector<IngredientUsage> list;
            while (getline(f, line)) {
                if (line.empty()) continue;
                if (line.rfind("ING|", 0) == 0) {
                    stringstream ss(line.substr(4));
                    string ingName; double used;
                    getline(ss, ingName, '|');
                    ss >> used;
                    IngredientUsage iu;
                    Ingredient* ref = stock.findByName(ingName);
                    if (ref != NULL) {
                        iu.ingredient = ref;
                        iu.usedKg = used;
                        list.push_back(iu);
                    ...
                    if (!name.empty()) {
                        menu.push_back(Dish(name, price, info, list));
                    }
                    list.clear();
                    name.clear();
                    info.clear();
            ...
            cout << Color::RED << "Error loading menu: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void showMenu() const {
        if (menu.empty()) {
            cout << Color::YELLOW << "Menu is empty!" << Color::RESET << endl;
        }
        else {
...
public:
    ...
    void removeDish(const string& name) {
        for (auto it = items.begin(); it != items.end(); ++it) {
            if (equalsIgnoreCaseASCII(it->dish.getName(), name)) {
                items.erase(it);
...
public:
    ...
    void setName(string n) {
        if (n.empty())
            throw string(Color::RED + "Name cannot be empty!" + Color::RESET);

        if (!isAlphaOnly(n))
            throw string(Color::RED + "Name must contain only letters!" + Color::RESET);

        // Ən azı 2 hərf olmalıdır
        int letterCount = 0;
        for (size_t i = 0; i < n.size(); i++) {
            if ((n[i] >= 'A' && n[i] <= 'Z') || (n[i] >= 'a' && n[i] <= 'z')) {
                letterCount++;
        ...
        if (letterCount < 2)
            throw string(Color::RED + "Name must contain at least 2 letters!" + Color::RESET);
...
public:
    void saveUsersToFile() {
        try {
            ...
            if (!f.is_open()) {
                throw runtime_error("Could not open users.txt file!");
            }
            ...
            cout << Color::RED << "Error saving users: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void loadUsersFromFile() {
        try {
            ...
            cout << Color::RED << "Error loading users: " << e.what() << Color::RESET << endl;
        }
    }
    ...
    void signUp() {
        ...
        a = getValidatedAge(Color::CYAN + "Age: " + Color::RESET);
                ...
                while (true) {
                    ...
                    try {
                        if (choice == 1) admin.showMenu();
                            ...
                            string name;
                            cout << Color::CYAN << "To remove: " << Color::RESET;
                            getline(cin, name);
                            ...
                            newQty = getValidatedDouble(Color::CYAN + "New quantity (kg): " + Color::RESET, 0.001, 100);
